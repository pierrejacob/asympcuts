...
#'@note Causal inference where both modules are logistic regressions performed
#'using PBMI; we use normal prior centred at 0 for both modules
...
#' 
cut_model_logistic_reg_bootstrap <- function(N,
                                             x,
                                             treatment,
                                             outcome,
                                             prior_sds_1,
  ...
  result_list <-  mclapply(1:N, function(iter){
    ...
    start_beta <- sapply(1:p, function(j) rnorm(1, mean = 0, sd = prior_sds_1[j]/2))
    ...
    function_to_optim <- function(beta){
      weighted_log_lik <- weighted_logistic_log_likC(beta, outcome, x_part_2, weights, p_2, n) 
    ...
    start_beta <- sapply(1:p_2, function(j) rnorm(1, mean = 0, sd = prior_sds_2[j]/2))
    optim_part_2 <- tryCatch(optim(par = start_beta, 
                                   fn = function_to_optim,
                                   method = 'BFGS',
                                   control = control), 
                             error = function(e)  return(list(error = TRUE)))
  ...
  beta_2_df <- do.call('rbind', lapply(result_list, function(x) x$beta_2))
  colnames(beta_2_df) <- paste0('V', 1:ncol(beta_2_df))
...
cut_model_linear_reg_bootstrap <- function(N,
                                                    x,
                                                    treatment,
                                                    outcome,
                                                    prior_sds_1,
  ...
  result_list <-  mclapply(1:N, function(iter){
    ...
    weights <- rexp(n, 1)
    function_to_optim <- function(beta){
      weighted_log_lik <- weighted_linear_regressionC(beta, treatment, x, weights, p, n) 
      log_prior <- sum(sapply(1:p, function(j) dnorm(beta[j], 
                                                     mean = 0, 
                                                     sd = prior_sds_1[j], 
                                                     log = TRUE)))
    ...
    if(ps_continuous){
      ...
      x_part_2 <- model.matrix(~prop_scores_factor) %>% 
        ...
        as.matrix %>% 
        unname()
      x_part_2 <- cbind(rep(1, n), x_part_2[,-1])
    }
...
linear_pb <- function(N,
  ...
  p <- ncol(x)
  n <- nrow(x)
  res_list <- mclapply(1:N, function(iter){
    weights <- rexp(n, 1)
    
    function_to_optim <- function(beta){
      weighted_log_lik <- weighted_linear_regressionC(beta, y, x, weights, p, n) 
      #weighted_log_lik <- -sum(weights*(y- as.vector(x%*%beta))^2)
      log_prior <- sum(sapply(1:p, function(j) dnorm(beta[j], 
                                                     mean = 0, 
                                                     sd = prior_sds[j], log = TRUE)))
      -weighted_log_lik - w_0*log_prior
    }
    ...
    if(isTRUE(optim_part_2$error)){
      ...
      beta <- optim_part_2$par
    }
    return(beta)
  }, mc.cores = nr_cores)
  
  matrix(unlist(res_list), ncol = p, byrow = TRUE) %>% as.data.frame()
}
